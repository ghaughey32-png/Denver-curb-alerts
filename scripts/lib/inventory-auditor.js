const { clipPathToDenver } = require("./denver-city-limits.js");
...
// Pink says "Denver has no schedule for this block, so you do not need to move
// your car", which is only ever true of curb Denver sweeps. A block that reaches
// across the city line is still Denver's for the part inside it, so the fallback
// is drawn from the trimmed geometry rather than the whole way — see
// clipPathToDenver for why the cut sits 20 m out. A path that leaves Denver and
// comes back yields one route per surviving piece; the first keeps the block's
// id so already-published ids stay stable.
function makeUnavailableRoutes(block) {
  const pieces = clipPathToDenver(block.geometry);
  // Only a block wholly outside Denver clips away to nothing, and the importer
  // excludes those before the audit ever sees them. Drawing the untrimmed way
  // is the safer answer if one slips through: a blank public block breaks the
  // product invariant, while a little pink past the line is what shipped before.
  const paths = pieces.length ? pieces : [block.geometry];

  return paths.map((path, index) => ({
    id: index === 0 ? `unavailable-${block.id}` : `unavailable-${block.id}-part-${index + 1}`,
...
function auditInventory({ routes, blocks, matchToleranceMeters = 12, minimumCoverage = 0.9, generateUnavailable = true }) {
  ...
  for (const block of blocks) {
    ...
    if (existingUnavailable) {
      ...
      const unavailable = makeUnavailableRoutes(block);
      generatedRoutes.push(...unavailable);
      unavailable.forEach((route) => candidates.push({ route, bounds: pathBounds(route.map.path) }));
      blockResults.push({ id: block.id, streetName: block.streetName, from: block.from, to: block.to, status: "unavailable", coverage, routeIds: unavailable.map((route) => route.id), generated: true });
    } else {
...
module.exports = { auditInventory, makeUnavailableRoutes, normalizeStreetName, routeHasUsableSchedule };
