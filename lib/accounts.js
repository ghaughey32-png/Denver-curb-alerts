...
function buildAccountRecord({ email, passwordHash, now = new Date() }) {
  ...
  return {
    ...
    billing: buildDefaultBilling()
  };
}

// Present from the first account so the payment work is a matter of filling these in rather than
// migrating every existing row. The shape follows Stripe's subscription object because that is what
// the webhook will hand us: status is Stripe's own vocabulary, not ours.
