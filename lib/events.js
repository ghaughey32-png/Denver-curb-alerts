...
// The funnel, in order, then what happened each time the plans were shown. A name on neither list
// is refused, so a typo or a curious caller cannot grow the collection without bound.
const FUNNEL_EVENTS = ["app_open", "curb_opened", "remind_tapped", "paywall_shown"];
const PAYWALL_OUTCOMES = ["trial_started", "subscription_started", "paywall_closed"];
...
// paywall outcome its share of the times the plans were shown. Those shares are where people are lost.
