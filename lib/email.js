...
//
//   - No image. Most clients block remote images by default, and a logo that does not load is a
//     broken box where the identity was supposed to be, which reads worse than no logo at all.
//     A text wordmark in the brand colours always renders, in every client, at every width.
...
function wrapHtml(lines) {
  ...
  return [
    ...
    // Wordmark, plus a short accent rule under it so the mail has a colour of its own at a glance.
    `<tr><td style="padding:24px 24px 0 24px;font-family:${BRAND_FONT}">`,
    `<div style="font-size:17px;font-weight:700;color:${BRAND_INK};letter-spacing:-0.01em">Denver Curb Alerts</div>`,
    `<div style="margin-top:10px;width:44px;height:3px;background-color:${BRAND_ACCENT};font-size:0;line-height:0">&nbsp;</div>`,
    "</td></tr>",
